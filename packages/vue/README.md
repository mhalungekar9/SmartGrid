...
The Vue package supports the shared core grid surface used in the playground: presets, overlays, sorting, filtering, editing, formulas, selection, range fill, undo/redo, column tools, grouping, pivoting, tree data, master/detail, export, AI actions, and server-side operation events. React-only productivity panels and behaviors such as Data Health, Trust Mode, and Collaboration are planned for parity work before those docs are promoted for Vue.
